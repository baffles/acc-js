...
^ code
	*/
	...
	post = post.replace(/<p>/gi, "");
	post = post.replace(/<\/p>/gi, "\n\n");
	post = post.replace(/\r|\n/g, "<br />"); // because JS regexes are CRIPPLED!

	post = post.replace(/<span class="ref">.*<\/span>/gi, "");
	post = post.replace(/<div class="ref-block">.*<\/div>/gi, "");
	post = post.replace(/<object.*\/object>/gi, "[Embedded Content Removed]");
	post = post.replace(/<span class="json">.*<\/span>/gi, "");
	post = post.replace(/<pre class="terminal scroll">(.*?)<\/pre>/gi, "<terminal>$1</terminal>");
	post = post.replace(/<div class="spoiler"><p>(.*?)<\/p><\/div>/gi, "<spoiler>$1</spoiler>");
...
*/
	...
	post = post.replace(/<\/div>/gi, ""); // remove extra divs, mostly from quote filtering
